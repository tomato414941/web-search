...
class TestAPISearchWithKey:
    ...
    def test_search_with_invalid_key_returns_401(self):
        ...
        with patch("web_search_frontend.api.deps.validate_api_key", return_value=None):
            client = TestClient(app)
            resp = client.get(
                "/api/v1/search",
                params={"q": "test"},
    ...
    def test_search_with_valid_key_returns_usage(self):
        ...
        with patch("web_search_frontend.api.deps.validate_api_key") as mock_validate:
            ...
            with patch("web_search_frontend.api.deps.get_daily_usage", return_value=5):
                client = TestClient(app)
                resp = client.get(
                    "/api/v1/search",
                    params={"q": "test"},
    ...
    def test_search_with_key_via_query_param(self):
        ...
        with patch("web_search_frontend.api.deps.validate_api_key") as mock_validate:
            ...
            with patch("web_search_frontend.api.deps.get_daily_usage", return_value=0):
                client = TestClient(app)
                resp = client.get(
                    "/api/v1/search",
