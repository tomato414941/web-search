...
from web_search_crawler.db.url_domain_state import MAX_DOMAIN_BACKOFF_SEC
...
class UrlFrontierMixin:
    ...
    def _update_domain_state_after_result(
        self,
        cur,
        *,
        domain: str,
        ...
        if is_success:
            ...
            if cur.rowcount == 0:
                self.domain_scheduling_state.ensure_missing_domain_state_rows(
                    cur,
                    [domain],
                    now=now,
    ...
    def record_frontier_result(self, url: str, status: str) -> None:
        ...
        with db_transaction(self.db_path) as cur:
            ...
            self._update_domain_state_after_result(
                cur,
                domain=domain,
                policy=policy,
