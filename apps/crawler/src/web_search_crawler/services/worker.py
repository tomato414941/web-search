...
class WorkerService:
    ...
    def _handle_task_done(self, task: asyncio.Task) -> None:
        ...
        exc = task.exception()
        if exc is not None:
            logger.error("Background worker crashed: %s", exc, exc_info=exc)
        else:
            logger.warning("Background worker stopped unexpectedly")
    ...
    async def start(self, concurrency: int = 1):
        ...
        from web_search_crawler.workers.tasks import worker_loop
