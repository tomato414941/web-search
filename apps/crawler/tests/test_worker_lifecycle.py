...
from web_search_crawler.workers.manager import WorkerManager
...
async def test_worker_service_start():
    ...
    with patch("web_search_crawler.workers.tasks.worker_loop", new_callable=AsyncMock):
...
@pytest.mark.asyncio
async def test_worker_service_marks_stopped_when_task_crashes():
    service = WorkerService()

    async def boom(*args, **kwargs):
        raise RuntimeError("boom")

    with patch("web_search_crawler.workers.tasks.worker_loop", side_effect=boom):
        await service.start(concurrency=1)
        assert service.is_running is True
